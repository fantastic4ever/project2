...
def send_to_request_queue(qName, op, response_queue_url, body, service_url, headers):
...
def get_finance_info():
    ...
    response, request_id = send_to_request_queue(queue_finance_name, 'GET', url, 'None', '/private/finance', request.headers)
...
def get_finance_info_by_studentid(studentid):
    client_id = request.headers.get('client_id')
    url = get_response_queue_url(client_id)
    response, request_id = send_to_request_queue(queue_finance_name, 'GET', url, 'None', '/private/finance/' + studentid, request.headers)
    return Response('{"_status": "SUCCESS", "_success": {"message":' + json.dumps(response) + ', "request_id":"' + request_id + '", "code": 200}}', mimetype='application/json', status=200)
...
def post_finance_info():
    ...
    response, request_id = send_to_request_queue(queue_finance_name, 'POST', url, str_json, '/private/finance', request.headers)
    return Response('{"_status": "SUCCESS", "_success": {"message":' + json.dumps(response) + ', "request_id":"' + request_id + '", "code": 200}}', mimetype='application/json', status=200)
...
def post_finance_info_by_studentid_courses(studentid):
    ...
    content = request.get_json(force = True)
    str_json = json.dumps(content)
    response, request_id = send_to_request_queue(queue_finance_name, 'POST', url, str_json, '/private/finance/' + studentid + '/courses', request.headers)
    return Response('{"_status": "SUCCESS", "_success": {"message":' + json.dumps(response) + ', "request_id":"' + request_id + '", "code": 200}}', mimetype='application/json', status=200)
...
def update_finance_info(studentid):
    ...
    content = request.get_json(force = True)
    str_json = json.dumps(content)
    response, request_id = send_to_request_queue(queue_finance_name, 'PUT', url, str_json, '/private/finance/' + studentid, request.headers)
    return Response('{"_status": "SUCCESS", "_success": {"message":' + json.dumps(response) + ', "request_id":"' + request_id + '", "code": 200}}', mimetype='application/json', status=200)
...
def update_finance_info_by_studentid_courses(studentid):
    ...
    content = request.get_json(force = True)
    str_json = json.dumps(content)
    response, request_id = send_to_request_queue(queue_finance_name, 'PUT', url, str_json, '/private/finance/' + studentid + '/courses', request.headers)
    return Response('{"_status": "SUCCESS", "_success": {"message":' + json.dumps(response) + ', "request_id":"' + request_id + '", "code": 200}}', mimetype='application/json', status=200)
...
def delete_finance_info():
    client_id = request.headers.get('client_id')
    url = get_response_queue_url(client_id)
    response, request_id = send_to_request_queue(queue_finance_name, 'DELETE', url, 'None', '/private/finance', request.headers)
    return Response('{"_status": "SUCCESS", "_success": {"message":' + json.dumps(response) + ', "request_id":"' + request_id + '", "code": 200}}', mimetype='application/json', status=200)
...
def delete_finance_info_by_studentid(studentid):
    client_id = request.headers.get('client_id')
    url = get_response_queue_url(client_id)
    response, request_id = send_to_request_queue(queue_k12_name, 'DELETE', url, 'None', '/private/finance/studentid/' + studentid, request.headers)
    return Response('{"_status": "SUCCESS", "_success": {"message":' + json.dumps(response) + ', "request_id":"' + request_id + '", "code": 200}}', mimetype='application/json', status=200)
...
def delete_finance_info_by_studentid_courses(studentid):
    client_id = request.headers.get('client_id')
    url = get_response_queue_url(client_id)
    response, request_id = send_to_request_queue(queue_k12_name, 'DELETE', url, 'None', '/private/finance/studentid/' + studentid + '/courses', request.headers)
    return Response('{"_status": "SUCCESS", "_success": {"message":' + json.dumps(response) + ', "request_id":"' + request_id + '", "code": 200}}', mimetype='application/json', status=200)
...
def get_k12_info():
    client_id = request.headers.get('client_id')
    url = get_response_queue_url(client_id)
    response, request_id = send_to_request_queue(queue_k12_name, 'GET', url, 'None', '/private/k12', request.headers)
    return Response('{"_status": "SUCCESS", "_success": {"message":' + json.dumps(response) + ', "request_id":"' + request_id + '", "code": 200}}', mimetype='application/json', status=200)
...
def get_k12_info_by_studentid(studentid):
    client_id = request.headers.get('client_id')
    url = get_response_queue_url(client_id)
    response, request_id = send_to_request_queue(queue_k12_name, 'GET', url, 'None', '/private/k12/studentid/' + studentid, request.headers)
    return Response('{"_status": "SUCCESS", "_success": {"message":' + json.dumps(response) + ', "request_id":"' + request_id + '", "code": 200}}', mimetype='application/json', status=200)
...
def get_k12_info_by_studentid_schoolid(studentid, schoolid):
    client_id = request.headers.get('client_id')
    url = get_response_queue_url(client_id)
    response, request_id = send_to_request_queue(queue_k12_name, 'GET', url, 'None', '/private/k12/studentid/' + studentid + '/schoolid/' + schoolid, request.headers)
    return Response('{"_status": "SUCCESS", "_success": {"message":' + json.dumps(response) + ', "request_id":"' + request_id + '", "code": 200}}', mimetype='application/json', status=200)
...
def post_k12_info():
    ...
    content = request.get_json(force = True)
    str_json = json.dumps(content)
    response, request_id = send_to_request_queue(queue_k12_name, 'POST', url, str_json, '/private/k12', request.headers)
    return Response('{"_status": "SUCCESS", "_success": {"message":' + json.dumps(response) + ', "request_id":"' + request_id + '", "code": 200}}', mimetype='application/json', status=200)
...
def update_k12_info(studentid, schoolid):
    ...
    content = request.get_json(force = True)
    str_json = json.dumps(content)
    response, request_id = send_to_request_queue(queue_k12_name, 'PUT', url, str_json, '/private/k12/studentid/' + studentid + '/schoolid/' + schoolid, request.headers)
    return Response('{"_status": "SUCCESS", "_success": {"message":' + json.dumps(response) + ', "request_id":"' + request_id + '", "code": 200}}', mimetype='application/json', status=200)
...
def delete_k12_info():
    client_id = request.headers.get('client_id')
    url = get_response_queue_url(client_id)
    response, request_id = send_to_request_queue(queue_k12_name, 'DELETE', url, 'None', '/private/k12', request.headers)
    return Response('{"_status": "SUCCESS", "_success": {"message":' + json.dumps(response) + ', "request_id":"' + request_id + '", "code": 200}}', mimetype='application/json', status=200)
...
def delete_k12_info_by_studentid_schoolid(studentid, schoolid):
    client_id = request.headers.get('client_id')
    url = get_response_queue_url(client_id)
    response, request_id = send_to_request_queue(queue_k12_name, 'DELETE', url, 'None', '/private/k12/studentid/' + studentid + '/schoolid/' + schoolid, request.headers)
    return Response('{"_status": "SUCCESS", "_success": {"message":' + json.dumps(response) + ', "request_id":"' + request_id + '", "code": 200}}', mimetype='application/json', status=200)
